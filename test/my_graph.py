...
def plot_rtt(time_list, SampleRTT, EstimatedRTT, DeviationRTT, TimeoutInterval):
    ...
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig('/vagrant/tju_tcp/test/RTT_VS_Time.png', dpi=600)
...
def plot_seq(time_list, seq_list):
    plt.plot(time_list, seq_list)
    plt.scatter(time_list, seq_list, s=5)
    plt.xlabel('Time (s)', fontdict={'size':font_size})
    plt.ylabel('Sequence Number', fontdict={'size':font_size})
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
...
plot_seq(SEND_dic['time'][:300],SEND_dic['seq'][:300])
